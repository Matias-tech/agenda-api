-- Migración corregida para gestión de correos electrónicos y proyectos API
-- Fecha: 2025-06-05
...
-- Solo agregar api_service a services si no existe
SELECT
    CASE
        WHEN COUNT(*) = 0 THEN 'ALTER TABLE services ADD COLUMN api_service TEXT;'
        ELSE 'SELECT "Column api_service already exists in services";'
    END as sql_command
FROM pragma_table_info ('services')
WHERE
    name = 'api_service';

-- Solo agregar api_service a availability si no existe
SELECT
    CASE
        WHEN COUNT(*) = 0 THEN 'ALTER TABLE availability ADD COLUMN api_service TEXT;'
        ELSE 'SELECT "Column api_service already exists in availability";'
    END as sql_command
FROM pragma_table_info ('availability')
WHERE
    name = 'api_service';
